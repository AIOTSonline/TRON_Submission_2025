...
// This tells the browser to wait until the entire HTML page is loaded before running the code inside.
document.addEventListener('DOMContentLoaded', (event) => {
    ...
    const testCodeButton = document.getElementById('test-code-btn');
    const compilerOutput = document.getElementById('compiler-output');
    const generatedCodeOutput = document.getElementById('c_text');
    ...
    // 1. Function for Testing the Blockly-generated code
    async function handleTestCode() {
        const codeToTest = generatedCodeOutput.textContent;
        if (!codeToTest || codeToTest.trim() === "") {
            alert("There is no code generated by Blockly to test.");
            return;
        }

        compilerOutput.textContent = "Sending code to compiler...";
        try {
            const response = await fetch('http://127.0.0.1:8000/test-code', {
                method: 'POST',
            ...
            compilerOutput.textContent = result.output;
        } catch (error) {
            console.error("Error testing code:", error);
            compilerOutput.textContent = `An error occurred: ${error.message}`;
        }
    }

    // 2. Function for sending context to the AI
    async function handleSendChat() {
        const userPrompt = chatPromptInput.value.trim();
        const initialCode = generatedCodeOutput.textContent;
        const logs = compilerOutput.textContent;
        ...
        if (!initialCode) {
            alert("There is no generated code to correct.");
            return;
        }

        addMessageToUI(userPrompt, 'user');
        chatPromptInput.value = '';
        addMessageToUI("typing...", 'model');

        try {
            const response = await fetch('http://127.0.0.1:8000/correct-code-chat', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    code: initialCode,
            ...
            chatHistory.push({ role: "model", parts: [result.chat_response] });

            chatHistoryDiv.removeChild(chatHistoryDiv.lastChild);
            addMessageToUI(result.chat_response, 'model');
            correctedCodeOutput.textContent = result.corrected_code;
        } catch (error) {
    ...
    // --- Event Listeners ---
    if (sendButton) sendButton.addEventListener('click', handleSendChat);
    if (chatPromptInput) chatPromptInput.addEventListener('keydown', (e) => { if (e.key === 'Enter') handleSendChat(); });
    if (testCodeButton) testCodeButton.addEventListener('click', handleTestCode);
});